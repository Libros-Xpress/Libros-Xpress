...
- **Objetivo:** Evitar la venta de productos agotados sincronizando el inventario en tiempo real y proporcionar a los clientes facturas digitales en formato PDF como comprobante de compra.
...
- Al registrar una venta en el punto físico, el stock en la web debe disminuir automáticamente.
...
- La venta física genera un pedido con estado "Venta Física" para mantener la trazabilidad.
...
**Para:** Proporcionar al cliente un respaldo legal de su compra y facilitar la organización contable  

#### Criterios de Aceptación
- Tras cada compra exitosa (en línea o física), el sistema debe generar automáticamente una factura en PDF.
- El archivo PDF debe contener: número de factura, fecha, cliente, detalle de productos, subtotal, impuesto, descuento y total.
- Desde el historial de pedidos, el usuario debe poder descargar la factura de cualquier pedido anterior.
- El nombre del archivo debe seguir el formato `factura_[id].pdf` y guardarse en la carpeta `facturas/`.
...
- Ventana `QMainWindow` con:
  - `QComboBox` que lista todos los productos con su stock actual.
